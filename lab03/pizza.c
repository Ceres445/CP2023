...
/**
 * @brief Returns the cost of the pizza base based on its type: Regular (R) or Thin crust (T).
 *
...
/**
 * @brief Returns the additional cost for the selected toppings: Cheese (C), Veggies (V), Meat (M)
 *
...
/**
 * @brief Computes the total cost of the pizza given the choice of base, size, and toppings.
 *
 */
int pizza_cost(char base_type, char size_type, char toppings_type)
{
  // TODO implement the function
  return (base_cost(base_type) + size_cost(size_type) + toppings_cost(toppings_type));
  return 0;
}
...
int main()
{
  ...
  printf("Your bill for the %c%c%c pizza is Rs. %d\n", 
                  base_type, size_type, toppings_type, total);
